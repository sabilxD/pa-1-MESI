...
void matmul_simd(const float* A, const float* B, float* C,
    ...
    for (int i = 0; i < M; ++i) {
        ...
        for (int j = 0; j < N; j+=4) {
            ...
            C[static_cast<long>(i) * ldc + j] = (float)temp[3];
            C[static_cast<long>(i) * ldc + j+1] = (float)temp[2];
            C[static_cast<long>(i) * ldc + j+2] = (float)temp[1];
            C[static_cast<long>(i) * ldc + j+3] = (float)temp[0];
        }
    }
}
